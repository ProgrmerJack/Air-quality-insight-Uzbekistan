...
for city, (ecsv, icol, fcol, rcsv) in CITIES.items():
    ep = ecsv if os.path.isabs(ecsv) or ecsv.startswith("..") else os.path.join(OUT, ecsv)
    ep = os.path.normpath(os.path.join(OUT, ecsv)) if ecsv.startswith("..") else os.path.join(OUT, ecsv)
    ...
    with open(os.path.join(OUT, f"regional_index_{city.lower()}.csv"), "w", newline="", encoding="utf-8") as f:
...
with open(os.path.join(OUT, "regional_injustice_summary.csv"), "w", newline="", encoding="utf-8") as f:
