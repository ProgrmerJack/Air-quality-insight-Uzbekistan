...
ACAG (download once): https://sites.wustl.edu/acag/datasets/surface-pm2-5/  (V5.GL.05.02, 0.01 deg, monthly)
  -> place the annual/monthly .nc in data/pipeline/cache/acag/  then run this.
...
ACAG_DIR = os.path.join(ROOT, "data", "pipeline", "cache", "acag")
...
def clip_acag():
    ...
    if not files:
        print("No ACAG .nc in data/pipeline/cache/acag/ -- download first (see header)."); return
    for fp in files:
